...
5. **下载模块**：负责管理下载任务，包括任务调度和多实例浏览器控制，确保下载过程的高效执行。
...
- 连接ItChatHandler和下载模块，将下载完成的文件上传回指定的微信群组，实现计数功能。
...
- 负责管理下载任务，包括任务调度和多实例浏览器控制，确保下载过程的高效执行。
- 管理浏览器实例，协调多个下载任务，提升系统的处理能力和稳定性。
- 处理下载任务的添加、分配和管理，提升系统的扩展性和稳定性。

**逻辑**：

1. **初始化管理器**：创建多个浏览器实例，每个实例负责处理一定数量的下载任务，确保下载过程的并行化和高效性。
2. **任务分配**：通过轮询或其他调度算法，将下载任务分配给不同的浏览器实例，避免单个实例过载。
3. **任务调度**：管理下载任务的队列，确保任务按照优先级和顺序被处理。
4. **资源管理**：监控各个浏览器实例的状态，动态调整任务分配，确保系统资源的合理利用。
5. **错误处理与重试**：在下载任务失败时，协调其他实例进行任务重试，提升任务完成率。
6. **任务执行**：通过浏览器自动化工具执行具体的下载操作，确保文件正确下载并上传。
7. **通知与日志**：在下载过程中记录详细日志，并在发生错误时通过通知模块向管理员反馈。

**更新内容**：

- **多实例浏览器控制**：支持同时管理多个浏览器实例，提升下载任务的处理能力。
...
- **统一管理与调度**：将XKW和AutoDownloadManager合并为单一的下载模块，简化系统架构和管理。
...
1. **系统启动**：
   - 系统加载配置文件，初始化各个模块（ItChatHandler、MessageHandler、AutoClicker、Uploader、下载模块、ErrorHandler、Notifier、AdminCommandsHandler、PointManager）。
...
3. **文件下载与上传**：
   - AutoClicker通过下载模块处理下载任务，自动下载文件到本地指定目录。
   - 下载完成后，AutoClicker将文件路径和对应的`soft_id`传递给Uploader。
   - Uploader将文件上传到指定的微信群组，并更新下载计数和剩余量。

4. **下载任务管理**：
   - 下载模块协调多个下载实例，分配下载任务，确保任务的高效执行。
   - 下载实例执行具体的下载操作，记录下载状态并与系统同步。
...
6. **管理员命令处理**：
   - 管理员发送特定命令（如添加/删除接收者、更新剩余次数、查询接收者信息、查询日志、查询浏览器状态）。
...
- **新增**：
  - 更新`Uploader`模块，增强上传任务管理和错误处理功能。
  - 引入文件删除线程，自动清理已上传的文件，确保系统存储空间的有效利用。
  - 优化积分管理逻辑，确保积分扣除的准确性和一致性。
  - 提供更多管理员命令，如添加、删除接收者，更新剩余次数，以及查询接收者信息。

- **更新**：
  - `Uploader`模块实现多线程上传，支持批量发送和重试机制，提升上传效率和稳定性。
  - `Downloader`模块合并了原有的XKW和AutoDownloadManager，简化系统架构，提升下载任务的管理和执行能力。
  - `ErrorHandler`和`Notifier`模块集成更加紧密，确保在发生错误时能够及时通知管理员。
  - `PointManager`模块优化数据库操作，增强多线程环境下的数据一致性。

- **修复**：
  - 修复了上传过程中可能导致的文件重命名失败问题。
  - 修复了部分管理员命令未能正确执行的BUG。
  - 优化了下载任务失败后的重试逻辑，提升任务完成率。
