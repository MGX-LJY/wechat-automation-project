...
class Notifier:
    def __init__(self, config: dict):
        ...
        if not admins:
            logging.error("微信通知的管理员列表未配置或为空")
            raise ValueError("微信通知的管理员列表未配置或为空")
        elif len(admins) > 1:
            logging.warning("检测到多个管理员配置，将仅使用第一个管理员进行通知")
        ...
        if self.method == 'wechat':
            self.wechat_notifier = WeChatNotifier(self.admin)
        else:
    ...
    def notify(self, message: str, is_error: bool = False) -> bool:
        ...
        :param message: 要发送的消息内容
        :param is_error: 是否为错误通知（此版本不区分是否错误通知，统一发送给管理员）
        :return: 发送是否成功
        """
        if self.method == 'wechat' and self.wechat_notifier:
            return self.wechat_notifier.send_message(message)
        else:
    ...
    def notify_images(self, image_paths: List[str], is_error: bool = False) -> bool:
        ...
        """
        if self.method == 'wechat' and self.wechat_notifier:
            return self.wechat_notifier.send_images(image_paths)
        else:
            logging.warning(f"无法发送通知，未知的通知方法: {self.method}")
            return False
