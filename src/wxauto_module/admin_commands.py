...
class AdminCommandsHandler:
    def __init__(self, config, point_manager, notifier=None, browser_controller=None, error_handler=None):
        ...
        self.help_templates = {
            ...
            # 【实例管理命令】
            '13': "禁用全部实例",
            '14': "查询群组下载份数 <群组名称> <时间段>",
            '15': "查询个人下载份数 <个人名称> <时间段>",
            '16': "查询所有群组今天下载量",
            '17': "查询所有群组这周下载量",
            '18': "查询当前账号使用情况",
            '19': "查询所有实例状态",
            '20': "检查所有实例状态",
            '21': "设置实例需要管理员介入 <实例ID> <True/False>",
        }
        self.commands = {
            # 数据库命令
            ...
            # 配置文件命令
            'add_listen': r'^添加监听列表\s+(.+)$',
            'remove_listen': r'^删除监听列表\s+(.+)$',
                ...
                if match:
                    ...
                    try:
                        if cmd == 'update_individual_points':
                            ...
                            if success:
                                return f"群组 '{group_name}' 的剩余积分已更新，变化量为 {delta}。"
                            else:
                                return f"群组 '{group_name}' 的积分更新失败。"
                        ...
                        # 查询积分命令
                            ...
                            if points is not None:
                                return f"群组 '{group_name}' 当前的积分为 {points}。"
                            else:
                                return f"未找到群组 '{group_name}' 的积分信息。"
    ...
    def modify_listen_list(self, names: str, action: str) -> str:
        """添加或删除监听列表中的名称（包括群组和个人）"""
        try:
            items = [name.strip() for name in names.split(',') if name.strip()]
            wechat_config = self.config.get('wechat', {})
            listen_list = wechat_config.get('listen_list', [])

            if action == 'add':
                for item in items:
                    if item not in listen_list:
                        listen_list.append(item)
            elif action == 'remove':
                for item in items:
                    if item in listen_list:
                        listen_list.remove(item)
            else:
                return "未知的操作类型。"

            # 更新配置
            wechat_config['listen_list'] = listen_list
            ...
            message = f"已{'添加' if action == 'add' else '删除'}监听列表中的项：{', '.join(items)}"
            logging.info(message)
            return message
        except Exception as e:
            logging.error(f"修改监听列表时发生错误: {e}", exc_info=True)
            if self.error_handler:
                self.error_handler.handle_exception(e)
            return f"修改监听列表时发生错误: {e}"
    ...
    def get_help_message(self) -> str:
        ...
        help_message = (
            ...
            "=== 配置文件命令 ===\n"
            "7. 添加/删除监听列表 <添加/删除> <名称1>,<名称2>\n"
            "   示例：添加监听列表 添加 群组A,个人B\n"
            "   示例：添加监听列表 删除 群组A,个人B\n\n"
